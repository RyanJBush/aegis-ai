...
type JsonReport = { scan_id: number; generated_at: string; findings: unknown[] };
type SarifReport = { scan_id: number; sarif: Record<string, unknown> };
type RemediationChecklistResponse = { scan_id: number; checklist: string[] };
type SuppressionExport = { scan_id: number; suppression_keys: string[] };
...
export async function fetchVulnerabilityById(vulnId: string): Promise<Vulnerability> {
  return mapRawVulnerability(await getJson<RawVuln>(`/vulnerabilities/${vulnId}`));
}
export async function fetchScanTrends(days = 14): Promise<ScanTrendPoint[]> {
  return (await getJson<{ points: ScanTrendPoint[] }>(`/scanning/history/trends?days=${days}`)).points;
}
export async function fetchKpiSummary(): Promise<KpiSummary> { return getJson<KpiSummary>('/scanning/kpi/summary'); }
...
export async function runScanNow(target: string, payload: string, profile: ScanRecord['profile']): Promise<ScanRecord> {
...
export async function updateFindingWorkflow(vulnId: string, payload: { status: Vulnerability['status']; assigned_owner?: string | null; notes?: string | null }): Promise<Vulnerability> {
  const vuln = await patchJsonRequest<RawVuln, { status: Vulnerability['status']; assigned_owner?: string | null; notes?: string | null }>(`/vulnerabilities/${vulnId}/workflow`, payload);
  return mapRawVulnerability(vuln);
}
export async function acceptRisk(vulnId: string, reason: string): Promise<void> {
  await postJsonRequest<Record<string, never>, { reason: string }>(`/vulnerabilities/${vulnId}/risk-acceptance`, { reason });
}
...
export async function fetchRemediationChecklist(scanId: string): Promise<string[]> {
  return (await getJson<RemediationChecklistResponse>(`/scanning/${scanId}/remediation-checklist`)).checklist;
}
export async function fetchSuppressions(scanId: string): Promise<string[]> {
  return (await getJson<SuppressionExport>(`/scanning/${scanId}/suppressions`)).suppression_keys;
}
export async function fetchAuditLogs(limit = 25): Promise<AuditLogEntry[]> { return getJson<AuditLogEntry[]>(`/observability/audit-logs?limit=${limit}&offset=0`); }
export async function fetchRuleHistory(limit = 25): Promise<RuleChangeEntry[]> { return getJson<RuleChangeEntry[]>(`/observability/rule-history?limit=${limit}&offset=0`); }
...
function mapJob(job: RawScanJob): ScanJob {
  return {
    id: String(job.id), target: job.scan_id ? `scan-${job.scan_id}` : 'queued target', status: job.status, findings: 0,
    startedAt: job.started_at ?? job.created_at, duration: job.completed_at ? 'done' : 'pending', scanId: job.scan_id ? String(job.scan_id) : undefined,
  };
}
function mapScanRecord(scan: RawScanRecord): ScanRecord {
  return { id: String(scan.id), target: scan.target, profile: scan.profile, status: scan.status, findings: scan.vulnerabilities_found, createdAt: scan.created_at, durationMs: scan.duration_ms };
}
