...
def test_scanning_service_remediation_checklist_and_counterlike(db_session: Session) -> None:
    ...
    vuln_a = Vulnerability(
        ...
        cwe_id="CWE-200",
        title="Leaked secret",
        description="High-risk secret pattern detected in configuration snippet.",
        affected_endpoint="/config",
        evidence="token=secret",
        example_request="POST /scan {\"snippet\":\"token=secret\"}",
        example_response='{"status":"warning"}',
    ...
    vuln_b = Vulnerability(
        ...
        description="Sensitive values exposed through logs.",
        affected_endpoint="/logs",
        evidence="password in logs",
        example_request="GET /logs?level=debug",
        example_response='{"status":"warning"}',
